...
function stats(id) {
  return sedeStats[String(id)] ?? {mostradores: 0, atendidos: 0, enEspera: 0}
}
</script>
  ...
  <AppLayout title="Sedes" subtitle="Administración de sedes y locales">
    ...
    <div v-else class="sedes-grid">
      <div v-for="s in filtered" :key="s.id" class="sede-card card">
        ...
        <p class="sede-address">Dirección: {{ s.direccion }}, {{ s.distrito }}</p>
        <p class="sede-horario">Horario: {{ s.horario }}</p>
